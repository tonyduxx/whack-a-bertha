...
function resize() {
  // Edge-to-edge grid: no margin
  const cols = cfg.cols, rows = cfg.rows;
  const pad = 22, margin = 0;
  // Use original door size for aspect ratio
  const gridW = cols * cfg.doorW + (cols - 1) * pad;
  const gridH = rows * cfg.doorH + (rows - 1) * pad;
  const aspect = gridW / gridH;
  // Get container width (or window width minus padding)
  const container = document.querySelector('.wrap');
  let targetW = Math.min(900, (container ? container.offsetWidth : window.innerWidth - 28));
